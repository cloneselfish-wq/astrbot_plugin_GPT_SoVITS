...
| 自动转语音（无命令） | - | Bot 回复阶段自动转语音。触发条件：插件启用、命中 `auto.tts_prob` 概率、回复文本长度不超过长度上限（群聊看 `auto.max_msg_len`，私聊看 `auto.max_msg_len_private`，后者默认 `0` = 不限制）。回复中的图片、@、引用等非文本组件会原样保留；合成失败会自动退回发送原文 |
| 工具调用（无命令） | LLM Tool | `gsv_tts` 工具，由 bot 自行决定何时用语音说话 |
| `语音语言 <中文/日语/英语/韩语/默认>` | `tts_lang` | 切换或查看本会话的语音语言（只对开启了「合成前翻译」的音色有效） |
...
- 内容超过长度上限 → 拒绝并提示模型精简，或改用文字（群聊按 `auto.max_msg_len`，私聊按 `auto.max_msg_len_private`，私聊默认不限制）；
- 当前机器人未命中音色档案且已开启 `only_configured_bots` → 拒绝；
- 合成失败 / 服务不可达 → 返回指引让模型改用文字回复，不重复调用；
- 成功发出语音后打标记，**发送前钩子会跳过这一轮**，避免同一句回复既发语音又转一遍语音。

工具另有一个 `lang` 参数（`zh` / `ja` / `en` / `ko`）：用户明确要求换语言时，模型会自己带上，
例如「用中文说」会填 `zh`。留空表示说这个音色本来的语言。
...
| `client.timeout` | API 请求超时时间的**基准值**（秒） | 按「一句话」估的基准。插件会按待合成文本的长度自动放宽（中文约 4.5 字/秒、合成耗时约为音频时长的 2 倍，上限 600 秒），所以长文本不必手动调大这个值 |
...
| `max_msg_len` | **群聊**转语音的最大文本长度 | 默认 `150`。群聊里超过该值直接发文字。文字越长合成越慢（实测合成耗时约为音频时长的 2 倍），不建议设得过大。该值同时也是 `gsv_tts` 工具在群聊里的单次朗读上限 |
| `max_msg_len_private` | **私聊**转语音的最大文本长度 | 默认 `0` = **不限制**。私聊是一对一场景，长语音只影响对话双方，没必要掐断；想给自己留个刹车可以填大于 0 的值（例如 `300`）。长文本的请求超时会按长度自动放宽（上限 600 秒），不用担心被超时掐断 |
...
`auto_translate` 会让音色**固定**用自己的语言说话，但群里常有人想听中文，为此加了语言点播：

- **默认不变**：没人提要求时，仍按档案的 `text_lang` 发音（日语音色说日语）。
- **点名才切**：「说中文」「用中文说」「换成中文」「说人话」「听不懂日语」这类请求会让这次语音改用中文；
  「别说中文」「别说日语」这类否定式也能正确识别。
- **会记一小会儿**：明确的请求在本会话记住 `translate.switch_ttl_minutes` 分钟（默认 10），不用反复强调；
  模糊表达（例如只发「用中文」）只影响被点名的那一次，避免误判后整个会话跟着变。
- **指令兜底**：`语音语言 中文` / `语音语言 日语` / `语音语言 默认`，可手动切换与恢复。

要点：

- 只对**打开了 `auto_translate`** 的音色生效。中文音色（如刻晴）换个语言只会得到怪口音，不参与点播。
- 切到的语言与原文一致时不调翻译，直接用原文合成；不同才翻译。
- 关掉 `插件配置 -> 语音翻译配置 -> 允许群友点播语音语言` 可完全禁用，回到「恒定说外语」的旧行为。
- 识别只看用户发的那条消息（超过 60 字不看），长文里出现「中文」不会误触发。
...
1. `auto.tts_prob` 被调低（默认 `0.1`，约一成回复转语音）；
2. **群聊**里回复超过 `auto.max_msg_len`（默认 150 字）—— 日志会打印「群聊回复长度 N 字超过上限」，
   超长回复直接发文字。私聊默认不限制（`auto.max_msg_len_private` = `0`），不会被这一条拦住；
3. `only_llm_result=true` 且该消息不是 LLM 输出（如指令回复）；
4. 该机器人未命中音色档案，且开启了 `auto.only_configured_bots` —— 日志会打印「未配置音色档案」；
5. 本轮已经由 `gsv_tts` 工具发过语音 —— 属正常行为，避免重复发两段语音；
6. GPT-SoVITS 实例未启动或参考音频路径不可达 —— 日志会打印
   「语音服务不可用，本条改为发送文字」或「语音合成失败，改为发送原文」。
   前者说明连不上（服务没起 / 隧道断），后者说明服务在但合成报错；
7. 服务刚被判定为不可用，正处于 60 秒冷却期 —— 日志会打印「连不上，60s 内跳过语音」。
