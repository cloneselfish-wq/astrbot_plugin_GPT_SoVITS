...
| `tts_prob` | 回复转语音的概率 | `0 ~ 1`。默认 `1.0`，即**每条回复都转**；调低则只有部分回复转语音 |
| `max_msg_len` | 转语音的最大文本长度 | 默认 `150`。超过该值直接发文字。文字越长合成越慢（实测合成耗时约为音频时长的 2 倍），不建议设得过大 |
...
4. GPT-SoVITS 实例未启动或参考音频路径不可达 —— 此时日志会打印「语音合成失败，改为发送原文」。
